...
(function () {
	//workflow sidebar
	$('#sidebarCollapseBtn,.overlay,#sidebarCollapse .sidebarBtn').on('click',function () {
	...
	window.alert = function (text, yesCallback, noCallback) {
		...
		// 自定義警告視窗
		var alertdiv = '<div class="outAlert"><div id="alertDiv"><div class = "alertHeader"> <button type = "button"	class = "close"> × </button></div><div class="alertContent">' + text + '</div><br /><input type="submit" name="button" class="btnBlue btn btnYes" id="btnYes" value="確定"/><input  type="submit" name="button" class="btnWhite btn btnNo" id="btnNo" value=" 取消 "/></div></div>'
...
// workflow headerBtn-md

	$('.workflowMoreBtnClick').click(function () {
		$('.workflowMoreBtnClick').toggleClass('d-block').toggleClass('d-md-none');
		$('.headerBtn .d-md-block').toggleClass('d-none')
	})
